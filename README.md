...
- **Dependency supply chain** — typosquatting detection, minimum package age enforcement (< 48h), maintainer change/takeover detection, dependency confusion warnings, malicious install script analysis, obfuscation/low-popularity/missing-provenance signals, and **transitive dependency scanning** of the full npm tree when a `package-lock.json` is present (not just declared deps). Powered by [dep-scan](https://github.com/tkdtaylor/dep-scan); the image is built automatically on first scan (~30s, downloads a prebuilt release binary — no Rust toolchain needed) and reused thereafter.
